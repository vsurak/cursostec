...
### 4. Diseño del SDLC (Software Development Life Cycle)

El **SDLC** es un marco metodológico que define cómo se desarrolla software de manera estructurada, asegurando calidad, mantenimiento y escalabilidad. Abarca **todas las fases**: análisis, diseño, desarrollo, pruebas, despliegue y operación. Hoy, la integración de **herramientas modernas**, **metodologías ágiles** y **IA de desarrollo** permite acelerar los ciclos y mejorar la colaboración.


### 4.1. Documentación y elementos clave en el SDLC

En la práctica, documentar correctamente el SDLC es crucial para **evitar errores, reducir retrabajo y mantener la trazabilidad de decisiones**.

#### a) Requerimientos

* **Funcionales**: qué debe hacer el sistema (ej.: “los usuarios pueden enviar y recibir mensajes en tiempo real”).
* **No funcionales**: rendimiento, escalabilidad, seguridad, disponibilidad (ej.: “el sistema debe soportar 10,000 usuarios concurrentes”).
* **Herramientas**: Notion, Confluence, ClickUp para documentación centralizada.
...
* **Supabase**: permite crear bases de datos Postgres con APIs automáticas, autenticación y almacenamiento de archivos, acelerando la entrega de backends funcionales sin codificación extensa.

#### c) Planificación y backlog

* Definir **epics**, historias (stories) y tareas (tasks).
* **Epics**: grandes bloques funcionales que se descomponen en historias más pequeñas. Ej.: “Sistema de chat” como epic → historias: enviar mensaje, historial, notificaciones.
...
* Gestión de incidencias: Jira Service Management, PagerDuty.

#### 2. Metodologías ágiles y técnicas modernas

El desarrollo ágil permite iteraciones rápidas y entrega de valor continuo. Las metodologías más usadas incluyen Scrum, Kanban y desarrollo iterativo, con soporte de **herramientas y técnicas recientes**.
...
* Técnicas recientes:
  ...
  * *Planning Poker*: estimaciones colaborativas usando técnicas de gamificación.
  * *Definition of Done*: checklist que asegura que cada historia cumpla todos los criterios de calidad.
  * Integración con IA: Copilot sugiere boilerplate de código, pruebas unitarias o documentación de endpoints.

#### b) Kanban

* **Flujo continuo**, no sprints fijos.
* Tablero visual: columnas To Do → In Progress → Done.
* Límite WIP (Work in Progress) para optimizar throughput.
* Métricas: tiempo de ciclo, tiempo de espera, throughput, acumulado de trabajo.
* Herramientas: Trello, Jira Kanban Boards, Notion Boards.

#### c) Desarrollo iterativo

* Cada iteración entrega una versión funcional parcial que se retroalimenta con feedback del cliente.
* Herramientas modernas: Figma (UI/UX rápido), Supabase (backend rápido), GitHub Actions (CI/CD).

#### d) Pair Programming

* Dos desarrolladores trabajan en el mismo código: driver (escribe) y navigator (revisa y piensa estrategia).
* Herramientas: VSCode Live Share, CodeTogether, Tuple.

#### e) IA en desarrollo ágil

* Aplicaciones: generar código, pruebas, documentación, refactorizaciones.
* Limitaciones: no reemplaza comprensión de negocio, puede generar vulnerabilidades o errores lógicos.
...
### 3. Ejemplo práctico: App de citas basada solo en texto y chat

#### a) Diseño

* Arquitectura:

  * Backend: Node.js + Express + Supabase (DB, autenticación, storage).
  ...
  * Ticketing: Jira → epic “Sistema de chat”, stories: enviar mensajes, historial, notificaciones en tiempo real.

#### b) Desarrollo

* Supabase: crea endpoints automáticos para CRUD de usuarios y chats, autenticación lista para usar.
...
* Seguridad: OWASP ZAP y Snyk para análisis de vulnerabilidades.

#### d) Stage y Producción

* Stage: despliegue temporal con datos simulados.
* Producción: Docker + Kubernetes, monitorización Prometheus/Grafana.

#### e) CI/CD

* Pipeline: Push → tests → build → deploy staging → revisión manual → deploy prod.


### 4. Diseño de equipos y combinaciones de roles

#### a) Adaptación de open source

* Roles: 2 backend, 1 QA, 1 DevOps, 1 Product Owner.
* Técnicas: feature branching, code review, CI/CD.
* Herramientas: GitHub, Jira, Supabase para prototipado rápido y pruebas.
...
* Técnicas: Scrum, pair programming, sprints de 2 semanas, AI-assisted code.
...
| Backend Dev   | Sofía T.   | API, DB, lógica de negocio   | Node.js, Supabase          |
| Backend Dev   | Andrés V.  | Integraciones, scripts DB    | Supabase, GitHub           |
| Frontend Dev  | Camila P.  | UI móvil, componentes        | React Native, Figma        |
| QA Automation | Luis F.    | Tests unitarios, integración | Cypress, Playwright        |
...
**Escenario:** Mercado de trueque digital con chat entre usuarios. Imaginemos una plataforma web y móvil que permite a los usuarios ofrecer y solicitar productos o servicios a cambio de otros, sin usar dinero. El objetivo es facilitar el trueque de manera segura, ágil y organizada, con comunicación directa entre usuarios mediante chat

**Tareas del grupo:**

1. Elegir Scrum o Kanban y justificar.
2. Crear un Epic y 3 tickets con descripción, criterios de aceptación y asignación.
3. Asignar roles
4. Mencionar una herramienta moderna para acelerar el desarrollo (ej: Supabase).

**Dinámica:**

* 10 min → discusión en grupos. El profesor armará los grupos aleatoriamente. 
* 5 min → enviar resultados por medio del classroom
