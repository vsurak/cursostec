...
Usando el modelo hecho en el ejercicio #3, proceda a hacerle deploy a un motor de base de datos de su elección y a correr el script que haga el llenado de los 1000 registros. Seguidamente cree un programa en el lenguaje de su elección que secuencialmente proceda a hacer lo siguiente:

a) crear 40 hilos de ejecución y que cada hilo se conecte a la base de datos haciendo un select * sobre la tabla que tiene los 1000 registros. Haga una marca de tiempo antes de iniciar los hilos y otra hasta que todos los 40 hilos hayan obtenido el resultado de la base de datos. Calcule la diferencia de marcas de tiempo en milisegundos e imprimilas indicando que es la "Ejecución directa"

b) haga el mismo ejercicio del punto a, pero en esta ocasión configure un pool de conexiones de tamaño fijo de 5 conexiones. Imprima la salida de tiempo de ejecución como "Fixed pool size"

c) haga el mismo ejercicio del punto b, pero esta vez agregue una cache usando memcache o un hashtable al menos, donde el query mismo sea el key de la cache y retorne el resultado. Imprima la salida de tiempo de ejecución como "Pool and cache". 

Haga submit del ejercicio antes de terminar la clase adjuntando:
- script de creación de la base de datos y el script de llenado de los 1000 registros
- código fuente del programa hecho 
- screenshot con los resultados obtenidos 
